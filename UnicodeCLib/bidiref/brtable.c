 ...
 * 2021-Sep-23 kenw   Updates for Unicode 14.0.
 * 2022-Sep-30 kenw   Updates for Unicode 15.0.
 * 2023-Oct-03 kenw   Updates for Unicode 15.1.
 * 2024-Sep-25 kenw   Updates for Unicode 16.0.
 */
 ...
 *
 * MAINTENANCE NOTE: If any new CJK block has been added to thi
 * version, add an explicit test for its range in the following
 * cascade. Note that the tests are in increasing code point order,
 * and not necessarily by the age (or name) of the CJK extension.
 */
 ...
 *
 * MAINTENANCE NOTE: If any new RTL block has been defined for
 * this version, verify that it has not changed the default
 * for its range to bc=AL. If so, adjust the assignments of
 * defaults below.
 */
...
/*
 * MAINTENANCE NOTE: When adding a new version of UBA to
 * this code, add match entries for the appropriately
 * named UnicodeData.txt and BidiBrackets.txt.
 */
    ...
    if ( status == UNICODE_DATA )
    {
        ...
        else if ( version == UBA160 )
        {
            fileName = "UnicodeData-16.0.0.txt";
        }
        else if ( version == UBA151 )
        {
            fileName = "UnicodeData-15.1.0.txt";
        }
        else if ( version == UBA150 )
        {
            fileName = "UnicodeData-15.0.0.txt";
        }
        else if ( version == UBA140 )
        {
    ...
    else if ( status == BRACKET_DATA )
    {
        ...
        {
            fileName = "BidiBrackets-16.0.0.txt";
        }
        else if ( version == UBA151 )
        {
            fileName = "BidiBrackets-15.1.0.txt";
        }
        else if ( version == UBA150 )
        {
...
/*
 * MAINTENANCE NOTE: When adding a new version of UBA to this code,
 * update the note to indicate the correct version of the library.
 */
    if ( Trace ( Trace0 ) )
    {
        printf ( "Note:  Initialized bidiref 16.0.0 library for UBA version %s\n", GetUBAVersionStr() );
    }
