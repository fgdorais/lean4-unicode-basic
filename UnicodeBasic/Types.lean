...
@[inline]
def toAbbrev! (x : GC) : String := reprAux x true |>.head!
...
@[deprecated GC.ofAbbrev! (since := "1.3.0")]
...
@[deprecated GC.ofAbbrev! (since := "1.3.0"), inherit_doc GeneralCategory.ofAbbrev?]
...
def GeneralCategory.ofGC? (c : GC) : Option GeneralCategory :=
  (GC.reprAux c).head? >>= fun s => ofAbbrev? s
